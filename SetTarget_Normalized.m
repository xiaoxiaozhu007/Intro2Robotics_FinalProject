% A simple UI input for the target location in the configuration space
function [targetXYZ]=SetTarget_Normalized(startXYZ,A,theta_points)
        global b1 b2 b3
        figuresize = [1024, 768];
        %figure,vis3D(A),title('3D grid and obstacles initial settings');
        %f = figure('Name', 'Configuration Space (Obstacles only)', 'position', ...
        %           [200, 200, figuresize(1), figuresize(2)]);
        f = figure('Name', 'Configuration Space (Obstacles only)', ...
                   'Units', 'normalized');%, ...
                   %[200, 200, figuresize(1), figuresize(2)]);
        subplot(1, 2, 1);
        title('Configuration Space (Obstacles only)');
        %vis3D_collision(A,0.2,'red');
        vis3D_collision_scat(A,0.2,'red');

        % where you start (change to current position later
        voxel( [startXYZ(1) - 0.5, startXYZ(2) - 0.5, startXYZ(3) - 0.5],...
                   [1, 1, 1], 'green', 0.75); 
               
        %subplot(4, 1, 1);
        
        pos = -2;
        while pos==-2    
            %%% SLIDER 1
            b1 = uicontrol('Parent', f, 'Style', 'slider', 'Units', ...
                           'normalized', 'Position', ...
                          [0.60, 0.85, 0.3, 0.05], ...
                          'value', 1, 'min', 1, ...
                          'max', theta_points, 'Callback', @Slider1_Callback );

            uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.55, 0.85, 0.05, 0.05], ...
                      'String', 1);
            uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.90, 0.85, 0.05, 0.05], ...
                      'String', theta_points);
            uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.95, 0.85, 0.05, 0.05], ...
                      'String', get(b1,'Value') );

            %%% SLIDER 2
            b2 = uicontrol('Parent', f, 'Style', 'slider', 'Units', ...
                           'normalized', 'Position', ...
                          [0.60, 0.75, 0.3, 0.05], ...
                          'value', 1, 'min', 1, ...
                          'max', theta_points, 'Callback', @Slider2_Callback);

            uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.55, 0.75, 0.05, 0.05], ...
                      'String', 1);
            uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.90, 0.75, 0.05, 0.05], ...
                      'String', theta_points);
            uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.95, 0.75, 0.05, 0.05], ...
                      'String', get(b2,'Value') );

            %%% SLIDER 3
            b3 = uicontrol('Parent', f, 'Style', 'slider', 'Units', ...
                           'normalized', 'Position', ...
                          [0.60, 0.65, 0.3, 0.05], ...
                          'value', 1, 'min', 1, ...
                          'max', theta_points,  'Callback', @Slider3_Callback);

            uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.55, 0.65, 0.05, 0.05], ...
                      'String', 1);
            uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.90, 0.65, 0.05, 0.05], ...
                      'String', theta_points);
            uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.95, 0.65, 0.05, 0.05], ...
                      'String', get(b3,'Value') );

            %%% GO BUTTON
            uicontrol('Parent', f, 'Style', 'pushbutton', 'Units', 'normalized', ...
                      'Position', [0.65, 0.45, 0.20, 0.10], 'String', ...
                      'GO', 'Callback', @Button_Callback);

            beginGO = 0;

            hold off;
            drawnow;
            %targetXYZ = [round(b1.Value), round(b2.Value), round(b3.Value)];
            while (beginGO == 0)  %%%%% What does this do? %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                pause(0.25);
            %     hold off;
            %     voxel( [targetXYZ(1) - 0.5, targetXYZ(2) - 0.5, targetXYZ(3) - 0.5],...
            %            [1, 1, 1], 'blue', 0);
            %     targetXYZ = [round(b1.Value), round(b2.Value), round(b3.Value)];
            %     hold on;
            %     voxel( [targetXYZ(1) - 0.5, targetXYZ(2) - 0.5, targetXYZ(3) - 0.5],...
            %            [1, 1, 1], 'blue', 0.75);
            %     
            %    drawnow;
            end

            targetXYZ = [round(b1.Value), round(b2.Value), round(b3.Value)];

            % check that targetXYZ is an obstacle!
            pos=A(targetXYZ(1), targetXYZ(2), targetXYZ(3));
            if pos==-2
                disp('target is an obstacle!');   %%% How to make it display in the current window?
            end
                %voxel( [targetXYZ(1) - 0.5, targetXYZ(2) - 0.5, targetXYZ(3) - 0.5],...
                       %[1, 1, 1], 'black', 0.75);
                %return;
                %continue;
            %end
        end
        %% UI setting
    % Slider 1 callback
    function Slider1_Callback(object, ~)
        val = round(b1.Value);
        b1.Value = val;
        uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.95, 0.85, 0.05, 0.05], ...
                      'String', get(b1,'Value') );
    end

    % Slider 2 callback
    function Slider2_Callback(object, ~)
        val = round(b2.Value);
        b2.Value = val;
        uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.95, 0.75, 0.05, 0.05], ...
                      'String', get(b2,'Value') );
    end

    % Slider 3 callback
    function Slider3_Callback(object, ~)
        val = round(b3.Value);
        b3.Value = val;
        uicontrol('Parent', f, 'Style', 'text', 'Units', 'normalized', ...
                      'Position', [0.95, 0.65, 0.05, 0.05], ...
                      'String', get(b3,'Value') );
    end

    % Button callback
    function Button_Callback(object, ~)
        beginGO = 1;
    end
    

voxel( [targetXYZ(1) - 0.5, targetXYZ(2) - 0.5, targetXYZ(3) - 0.5],...
           [1, 1, 1], 'blue', 0.75);
disp('Valid input');
drawnow;
end