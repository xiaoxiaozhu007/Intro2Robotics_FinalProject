...
obstacleset = 'Set1_A'; %define which obstacle set to use
...
%obstacleset = strcat(obstacleset,  );
%obstacleset = strcat(obstacleset, );
...
% [25, 50, 16]
% []
targetXYZ=SetTarget_Normalized(startXYZ,A,theta_points);
%targetXYZ = [1, 2, ];
%% Breath fast search
[distanceBFS,pathPts] = BreathFastSearch(A,targetXYZ,theta_points,startXYZ);
vis3DPath(distanceBFS,pathPts,A); % debugging only

%%%%%%%%%% Configure to arduino
